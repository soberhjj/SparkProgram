...
object ReadFromKafkaAndStoreResultAndOffsetInHBase {
  def main(args: Array[String]): Unit = {
    ...
    val historyOffsets: Map[TopicPartition, Long] = HBaseOffsetUtils.queryHistoryOffsetFromHBase(groupId)
    ...
    kafkaDStream.foreachRDD(rdd => {
      if (!rdd.isEmpty()) {
        ...
        //过滤问题数据（如将为null的数据过滤掉，因为上面如果json解析失败的话那么返回的是null，所以过滤掉这些null）
        val filteredRDD = orderRDD.filter(_ != null)
        //将处理后的数据保存到HBase中
        filteredRDD.foreachPartition(iter => {
          //将RDD中每个分区的数据保存到HBase中
          if (iter.nonEmpty) {
            //创建一个HBase连接
            ...
            iter.foreach(order => {
              ...
              //当hbasePutList中每达到指定个数的Put对象就执行一次写入，即每一条数据执行一次写入。这样做的目的是防止一次写入过多导致出现一些意外情况
              if (hbasePutList.size() == 1) { //达到1个就执行一次写入
                //执行写入
